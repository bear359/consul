...
func (c *ConfigEntry) ResolveServiceConfig(args *structs.ServiceConfigRequest, reply *structs.ServiceConfigResponse) error {
	...
	var priorSignature state.ConfigEntryIndexSignature
	...
	return c.srv.blockingQuery(
		...
		func(ws memdb.WatchSet, state *state.Store) error {
			...
			// "signature" of the discovered config entries to determine if
			// this was a spurious wakeup or not.
			entries, err := state.ReadResolvedServiceConfigEntries(
				ws,
				args.Name,
				&args.EnterpriseMeta,
				upstreamIDs,
			...
			if !priorSignature.IsZero() {
				if priorSignature.IsSame(&entries.Signature) {
					c.logger.Trace("ResolveServiceConfig saw spurious wakeup",
						"service", structs.NewServiceID(args.Name, &args.EnterpriseMeta),
						"prev-index", reply.Index,
						"curr-index", entries.MaxIndex,
						"prev-sig", priorSignature,
						"next-sig", entries.Signature,
					)
					priorSignature = entries.Signature
					// NOTE: the prior response is still alive inside of *reply
					reply.Index = entries.MaxIndex
					return errNotChanged
				} else {
					c.logger.Trace("ResolveServiceConfig saw legit wakeup",
						"service", structs.NewServiceID(args.Name, &args.EnterpriseMeta),
						"prev-index", reply.Index,
						"curr-index", entries.MaxIndex,
						"prev-sig", priorSignature,
